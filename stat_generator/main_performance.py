...
def dump_result(performance_dict, cpu_dict):
        ...
        for queue_depth in queue_depth_array:
            for thread in thread_array:
                ...
                f.write(workload_string_identifier[count]+','+workload_identifier +
                        ...
                        str(performance_dict[workload_identifier][1]) + ',' +
                        str(mean)+','+str(variance)+"\n")
    ...
    #         print ("No performance data for "+key)
    #         continue
    #     nums = re.findall('[0-9]+', key)
    #
    #     queue_depth = nums[0]
    #     thread = nums[1]
    #     f.write(key+','+thread+','+queue_depth+','+str(performance_dict[key][0])+','+str(performance_dict[key][1])+'\n')
...
def parse_cpu_file(fname, N, result_dict):
    ...
    #print(prefix)
    nums = re.findall('[0-9]+', prefix)

    queue_depth = int(nums[0])
    thread = int(nums[1])
